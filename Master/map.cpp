...
void MapObj::Draw(void)
{
	...
	CEnemy  *Enemy  = CEnemy::Get_Enemy(0);
	D3DXMATRIX playermatrix = player->Get_Player_position();

	Sprite_Draw(TEX_MAP, 850.0f, 0.0f, 0.0f, 0.0f, 150, 150);

	Sprite_Draw(TEX_Player_Icon, 895.0f + playermatrix._41, 60.0f - playermatrix._43, 0.0f, 0.0f, 60, 30);

	for (int i = 0; i < ENEMY_MAX; i++)
	{
		Sprite_Draw(TEX_ENEMY_ICON, (SCREEN_WIDTH - 24) / 10 * 8 + 120.0f + Get_Enemy_position(i).x, 60.0f - Get_Enemy_position(i).z, 0.0f, 0.0f, 6, 6);
	}
}
